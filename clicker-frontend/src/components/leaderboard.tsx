      ...
      <div className="flex items-center gap-3">
        <span
          className={cn(
            ...
            isCurrentUser || isHighlighted ? 'text-white' : rank <= 3 ? 'text-yellow-500' : 'text-[var(--tg-hint,#999)]'
          )}
        >
          {rank}
        </span>
        <span
          className={cn('font-medium', isCurrentUser || isHighlighted ? 'text-white' : 'text-[var(--tg-text,#000)]')}
        >
          {isCurrentUser ? `${name} (You)` : name}
        </span>
      </div>
      <span className={cn('font-mono', isCurrentUser || isHighlighted ? 'text-white' : 'text-[var(--tg-text,#000)]')}>
        {formatScore(score)}
      </span>
    </div>
  );
